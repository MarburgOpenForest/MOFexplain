...
* vw_actual_tree_dendroband^*^
* vw_actual_tree_diameter
* vw_actual_tree_habitat^*^
...
^*^Views with concatenated datafield.

### All trees with actual parameters:
All living trees with the actual parameters and additional information from the species list. That is the view to visualize the active trees:
```sql
...
### Dead trees
All dead trees (standing and lying) are composed respectivly with:
```sql
...
All general information for a forest structure plot is determined with:
```sql
